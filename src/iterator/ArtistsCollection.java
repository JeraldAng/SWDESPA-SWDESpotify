...
public class ArtistsCollection implements Container {

    private Users[] users;

    public ArtistsCollection(Users user, String keyword){
        ArrayList<Object> bind = new ArrayList<>();
        bind.add(user.user_id);
        bind.add(keyword);
        ArrayList<Users> usersList = (ArrayList<Users>) user.find("user_type = 1 AND user_id <> ? AND username LIKE ?", bind);
